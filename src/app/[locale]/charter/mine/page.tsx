...
import { CharterRowActions } from "@/components/charter-row-actions";
import { countryName } from "@/lib/boats/constants";
import { getMyCharters } from "@/lib/charter/queries";
...
export default async function MyChartersPage({ params }: Props) {
  await params;
  ...
  const listings = (await getMyCharters()) as unknown as Array<{
    id: string;
    charter_type: string;
          ...
          {listings.map((l) => {
            ...
            return (
              <li key={l.id} className="rounded-lg border border-slate-200 p-3">
                ...
                <div className="mt-3">
                  <CharterRowActions id={l.id} status={l.status} labels={labels} />
                </div>
              </li>
            );
          })}
