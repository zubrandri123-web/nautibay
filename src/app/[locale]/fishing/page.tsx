...
export default async function FishingPage({ searchParams }: Props) {
    ...
    <div className="mx-auto max-w-5xl px-4 py-10">
      ...
      <form className="mt-6 grid grid-cols-2 gap-3 rounded-lg border border-slate-200 p-4 sm:grid-cols-4">
        <label className="text-xs font-medium text-slate-600">
          {t("tripType")}
          <select name="tripType" defaultValue={filters.tripType ?? ""} className={inputCls}>
        ...
        <label className="text-xs font-medium text-slate-600">
          {t("duration")}
          <select name="duration" defaultValue={filters.duration ?? ""} className={inputCls}>
              ...
              <li key={l.id}>
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    {l.trip_type && l.boat_name ? (
                      <p className="mt-0.5 text-sm text-slate-600">
                        {tTrip(l.trip_type as never)}
                      </p>
                    ) : null}
                    {l.boat_length_m ? (
                      <p className="mt-1 text-sm text-slate-500">
                        {formatLength(l.boat_length_m, tCommon("unitM"), tCommon("unitFt"))}
                        {l.max_anglers ? ` · ${t("upToAnglers", { count: l.max_anglers })}` : ""}
                      </p>
                    ) : l.max_anglers ? (
                      <p className="mt-1 text-sm text-slate-500">
                        {t("upToAnglers", { count: l.max_anglers })}
                      </p>
                    ) : null}
                    {place ? (
