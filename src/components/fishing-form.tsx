...
import { createFishingAction } from "@/app/[locale]/fishing/new/actions";
import { updateFishingAction } from "@/app/[locale]/fishing/[id]/edit/actions";
import {
  fishingListingSchema,
  type FishingListingFormValues,
  type FishingListingInput,
} from "@/lib/fishing/schema";
import {
...
type EditInitial = Partial<FishingListingFormValues> & { photoPaths?: string[] };
...
export function FishingForm({
  ...
  const t = useTranslations("FishingForm");
  const tTrip = useTranslations("TripType");
  const tDur = useTranslations("TripDuration");
  ...
  const {
    ...
    control,
    formState: { errors },
  } = useForm<FishingListingFormValues, unknown, FishingListingInput>({
    resolver: zodResolver(fishingListingSchema),
    defaultValues: {
      currency: "EUR",
      dimUnit: "m",
  ...
  async function onSubmit(values: FishingListingInput) {
    setServerError(null);
    setSubmitting(true);
    const toM = (v: number | undefined) =>
      v == null
        ...
        : values.dimUnit === "ft"
          ...
          : v;
    const payload = { ...values, boatLengthM: toM(values.boatLengthM) };
    const result = listingId
      ? await updateFishingAction(listingId, locale, payload)
      : await createFishingAction(locale, payload);
    setSubmitting(false);
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-8">
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Field label={t("tripType")} optional error={errors.tripType?.message}>
          ...
          <Field label={t("duration")} optional error={errors.duration?.message}>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <Field label={t("ratePeriod")} hint={t("ratePeriodHint")} optional error={errors.ratePeriod?.message}>
            <select {...register("ratePeriod")} className={input}>
              <option value="">—</option>
              {FISHING_RATE_PERIODS.map((v) => (
                <option key={v} value={v}>
                  {tRate(v)}
                </option>
              ))}
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label={tForm("lengthM")} optional error={errors.boatLengthM?.message}>
            <input type="number" step="any" inputMode="decimal" {...register("boatLengthM")} className={input} />
          </Field>
          <Field label={t("maxAnglers")} optional error={errors.maxAnglers?.message}>
            <input type="number" inputMode="numeric" {...register("maxAnglers")} className={input} />
          </Field>
        </div>
      </section>
      ...
      <section className="space-y-3">
        ...
        <label className={check}>
          <input type="checkbox" {...register("tackleIncluded")} /> {t("tackleIncluded")}
        </label>
        <label className={check}>
          <input type="checkbox" {...register("baitIncluded")} /> {t("baitIncluded")}
        </label>
        <label className={check}>
          <input type="checkbox" {...register("licenseIncluded")} /> {t("licenseIncluded")}
        </label>
        <label className={check}>
          <input type="checkbox" {...register("foodIncluded")} /> {t("foodIncluded")}
        </label>
        <label className={check}>
          <input type="checkbox" {...register("keepCatch")} /> {t("keepCatch")}
        </label>
        <label className={check}>
      ...
      <section className="space-y-4">
        ...
        <Field label={t("rules")} hint={t("rulesHint")} optional>
          <textarea rows={3} {...register("rules")} className={input} />
        </Field>
