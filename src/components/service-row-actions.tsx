"use client";

import { useState, useTransition } from "react";
import { useRouter } from "next/navigation";
import { useTranslations } from "next-intl";
import { Link } from "@/i18n/navigation";
import {
  bumpServiceAction,
  deleteServiceAction,
  setServiceStatusAction,
} from "@/app/[locale]/services/mine/actions";

type Labels = {
  edit: string;
  archive: string;
  resubmit: string;
  del: string;
  confirmDelete: string;
};

export function ServiceRowActions({
  id,
  status,
  labels,
}: {
  id: string;
  status: string;
  labels: Labels;
}) {
  const router = useRouter();
  const tCommon = useTranslations("Common");
  const [pending, startTransition] = useTransition();
  const [err, setErr] = useState<string | null>(null);

  const run = (fn: () => Promise<{ error?: string; hoursLeft?: number }>) =>
    startTransition(async () => {
      setErr(null);
      const res = await fn();
      if (res?.error) setErr(res.error);
      else if (res?.hoursLeft) {
        setErr(tCommon("bumpCooldown", { hours: res.hoursLeft }));
      } else router.refresh();
    });

  const btn =
    "rounded-md border border-slate-300 px-3 py-1.5 text-sm hover:border-slate-500 disabled:opacity-50";

  return (
    <div className="flex flex-wrap items-center gap-2">
      <Link href={`/services/${id}/edit`} className={btn}>
        {labels.edit}
      </Link>
      <button
        type="button"
        disabled={pending}
        onClick={() => run(() => bumpServiceAction(id))}
        className={btn}
      >
        {tCommon("bump")}
      </button>
      {status === "archived" ? (
        <button
          type="button"
          disabled={pending}
          onClick={() => run(() => setServiceStatusAction(id, "pending_review"))}
          className={btn}
        >
          {labels.resubmit}
        </button>
      ) : (
        <button
          type="button"
          disabled={pending}
          onClick={() => run(() => setServiceStatusAction(id, "archived"))}
          className={btn}
        >
          {labels.archive}
        </button>
      )}
      <button
        type="button"
        disabled={pending}
        onClick={() => {
          if (window.confirm(labels.confirmDelete)) run(() => deleteServiceAction(id));
        }}
        className={`${btn} border-red-300 text-red-700 hover:border-red-500`}
      >
        {labels.del}
      </button>
      {err ? <span className="text-xs text-red-600">{err}</span> : null}
    </div>
  );
}
